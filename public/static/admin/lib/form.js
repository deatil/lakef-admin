 $(function() {
     ...
     $('.js-upload-file,.js-upload-files').each(function() {
         ...
         var uploader = WebUploader.create({
             ...
             duplicate: true,
             // swf文件路径
             swf: images_url.webuploader_swf,
             // 文件接收服务端。
             server: images_url.file_upload_url,
             // 选择文件的按钮。可选。
         ...
         uploader.on('uploadSuccess', function(file, response) {
             ...
             if (response.code == 0) {
                 if ($multiple) {
                     if ($input_file.val()) {
                         $input_file.val($input_file.val() + ',' + response.data.id);
                     } else {
                         $input_file.val(response.data.id);
                     }
                     $li.find('.remove-file').attr('data-id', response.data.id);
                 } else {
                     $input_file.val(response.data.id);
                 }
             }
             // 加入提示信息
             $li.find('.file-state').html('<span class="text-' + response.data.type + '">' + response.message + '</span>');
             // 添加下载链接
             $li.find('.download-file').attr('href', response.data.path);
         });
    ...
    $('.js-upload-image,.js-upload-images').each(function() {
         ...
         var uploader = WebUploader.create({
             ...
             server: images_url.image_upload_url,
             // 图片限制大小
             fileSingleSizeLimit: $size,
             // 只允许选择图片文件。
             accept: {
         ...
         uploader.on('fileQueued', function(file) {
             var $li = $(
                     ...
                     '<i class="fa fa-trash-o remove-picture"></i>' +
                     ($multiple ? '<i class="fa fa-arrows move-picture"></i>' : '') +
         ...
         uploader.on('uploadSuccess', function(file, response) {
             ...
             if (response.code == 0) {
                 if ($multiple) {
                     ...
                     $li.find('.remove-picture').attr('data-id', response.data.id);
                 } else {
                     $input_file.val(response.data.id);
                 }
             }
             $li.find('.file-state').html('<div class="layui-bg-green">' + response.message + '</div>');
             $li.find('img').attr('data-original', response.data.path);
             // 上传成功后，再次初始化图片查看功能
            $('.uploader-list').each(function () {
                $(this).viewer();
            });
